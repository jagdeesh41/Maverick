...
// GET /rm/assets -> AssetService.getAllAssets() — oversight view across
// every customer, not just the RM's own. Freeze/unfreeze live on the
// asset detail page (POST /rm/assets/{id}/freeze|unfreeze).
export default function RmAssets() {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--lb-ink-soft)' }}>
        Live from <code>GET /rm/assets</code>. "Raise dispute" calls{' '}
        <code>POST /rm/inheritance/{'{assetId}'}/dispute</code> — this only
        works if the asset already has an inheritance policy set.
      </p>
      ...
      {assets && (
        <table className="lb-table">
          <thead>
            <tr><th>ID</th><th>Owner</th><th>Type</th><th>Value</th><th>Status</th><th></th></tr>
          </thead>
          <tbody>
            {assets.map((a) => (
              <tr key={a.id}>
